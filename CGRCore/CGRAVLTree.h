...
#ifndef CGRCore_CGRAVLTree_h
#define CGRCore_CGRAVLTree_h

#include "CGRLinearStack.h"
//#import <CGRCore/CGRCircularVector.h>



//Basic implementation of an AVL Tree
//Note: this is NOT thread safe

class CGRAVLTreeException{};

class CGRAVLTreeNotEnoughSpace:CGRAVLTreeException{};


template <class KEY>
struct CGRAVLTreeKeys
{
    uint32_t size;
    KEY *keys;
};
...
struct CGRAVLTreeNode
{
    KEY key;
    DATA *data;
    struct CGRAVLTreeNode *left, *right;
    uint32_t height;
    
    CGRAVLTreeNode(KEY k, DATA *d)
    {
        key = k;
        data = d;
        left = NULL;
...
private:
    ...
    CGRAVLTreeNode<KEY, DATA> *rootNode;
    
    CGRLinearStack<CGRAVLTreeNode<KEY, DATA> *> *defaultStack;
    
    uint32_t _numberOfNodes;
    
    inline void rotateLeft(CGRAVLTreeNode<KEY, DATA> *node)
    {
