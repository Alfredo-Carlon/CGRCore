...
@implementation CGRGeometricGraphDrawing
...
-(void)drawSpanningGraph:(CGRCircularVector<uint32_t> *)vertexSet
{
    ...
    for(int i=0; i != numberOfVertices; i++){
        ...
        for(uint32_t j=0; j != numberOfNeighbors; j++){
            ...
            if(neighbor > drawVertex ->ident || !verticesTree ->containsKey(neighbor))
                continue;
...
-(void)setUpGraph: (CGRGeometricGraph *)graph andGraphRectangle:(NSRect)gr
{
    ...
    //[associatedGraph addNewGeometricVertexNotify:self];
    [associatedGraph addGeometricNotifiy:self];
    mapTree = NULL;
    ////////////////////First test version....
    graphRectangle = gr;
...
-(void)drawToEPSFile:(const char *)path
{
    ...
    for(uint32_t i = 0; i != numberOfVertices; i++){
        ...
        for(uint32_t j=0; j != numberOfNeighbors; j++){
            ...
            /*if([associatedGraph customDataForEdge:makeEdge(drawVertex ->ident, neighbor) withNumber:3]){
                epsFile << "0.1 setlinewidth" << std::endl;
            }else{
                continue;
            }*/
            
            if(neighbor > drawVertex ->ident)
                continue;
    ...
    for(uint32_t i=0; i != numberOfVertices; i++){
        ...
        if([associatedGraph customDataForVertex:drawVertex ->ident withNumber:3]){
            epsFile << "0.3 setlinewidth" << std::endl;
            epsFile << "0.7 setgray" << std::endl;
            epsFile << screenXUnit*(drawVertex -> location.x - graphRectangle.origin.x) << " ";
            epsFile << screenYUnit*(drawVertex ->location.y - graphRectangle.origin.y) << " moveto" << std::endl;
            epsFile << screenXUnit*(drawVertex -> location.x - graphRectangle.origin.x) << " ";
            epsFile << screenYUnit*(drawVertex ->location.y - graphRectangle.origin.y) << " " ;
            epsFile << radius+8 << " 0 360 arc fill" <<std::endl;
            
            //ID Write Code
            epsFile << "0.0 setgray" << std::endl;
            epsFile << "/Times-Roman findfont"<<std::endl;
            epsFile << "10 scalefont" << std::endl;
            epsFile << "setfont" << std::endl;
            epsFile << "newpath" << std::endl;
            epsFile << screenXUnit*(drawVertex ->location.x - graphRectangle.origin.x) - 10 << " ";
            epsFile << screenYUnit*(drawVertex ->location.y - graphRectangle.origin.y) - 10 << " moveto" << std::endl;
            epsFile << "(" << drawVertex ->ident << ") show" << std::endl;
        ...
        epsFile << "setfont" << std::endl;
        epsFile << "newpath" << std::endl;
        epsFile << screenXUnit*(drawVertex ->location.x - graphRectangle.origin.x) - 1 << " ";
        epsFile << screenYUnit*(drawVertex ->location.y - graphRectangle.origin.y) - 1 << " moveto" << std::endl;
        epsFile << "(" << drawVertex ->ident << ") show" << std::endl;
...
-(void)updateVertexRender:(uint32_t)vertexInd
{
    double screenYUnit = screenRectangle.size.height / graphRectangle.size.height;
    double screenXUnit = screenRectangle.size.width / graphRectangle.size.width;
    ...
    struct CGRGeoGraphDrawingVertex *drawVertex = drawVertices -> elementAtIndex(vertexInd);
    if(drawVertex -> renderPoints == NULL){
        drawVertex ->numberOfSides = 12;
        if(drawVertex ->renderPoints == NULL){
            drawVertex ->renderPoints = (GLfloat *)calloc(drawVertex ->numberOfSides*3, sizeof(GLfloat));
...
#pragma mark -
#pragma mark CGRGeometricGraph notifications
...
-(void)vertexAdded:(uint32_t)vertexId atCoordinates:(NSPoint)cords
{
    if(cords.x >= graphRectangle.origin.x && cords.x <= graphRectangle.origin.x + graphRectangle.size.width)
        if(cords.y >= graphRectangle.origin.y && cords.y <= graphRectangle.origin.y + graphRectangle.size.height){
            struct CGRGeoGraphDrawingVertex *newDraw = new struct CGRGeoGraphDrawingVertex();
            initDrawingVertex(newDraw, vertexId, cords);
            uint32_t vertexIndx = drawVertices ->BSInserts(newDraw, false);
            [self updateVertexRender:vertexIndx];
        }
...
{
    //Check if the vertex is being displayed
    CGRGeoGraphDrawingVertex queryWrapper;
    queryWrapper.ident = vertex;
    bool displayed = drawVertices ->BSContains(&queryWrapper, false);
    //Check if the new coordinates could be displayed
    if(cords.x >= graphRectangle.origin.x && cords.x <= graphRectangle.origin.x + graphRectangle.size.width)
        if(cords.y >= graphRectangle.origin.y && cords.y <= graphRectangle.origin.y + graphRectangle.size.height){
            if(displayed){
                //We just need to update the renderings
                uint32_t vertexIndx = drawVertices ->BSIndexOfElement(&queryWrapper, false);
                CGRGeoGraphDrawingVertex *vert = drawVertices ->elementAtIndex(vertexIndx);
                vert ->location = cords;
                [self updateVertexRender:vertexIndx];
            }else{
                //We add the vertex
                struct CGRGeoGraphDrawingVertex *newDraw = new struct CGRGeoGraphDrawingVertex();
                initDrawingVertex(newDraw, vertex, cords);
                uint32_t vertexIndx = drawVertices ->BSInserts(newDraw, false);
                [self updateVertexRender:vertexIndx];
            }
            return;
    ...
    if(displayed){
        //The vertex is out of the diplay area
        uint32_t vertexIndx = drawVertices ->BSIndexOfElement(&queryWrapper, false);
        CGRGeoGraphDrawingVertex *vert = drawVertices ->removeAtIndex(vertexIndx);
        delete vert;
    }
